...
def _main(raw_args):
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Directory with the original executable files. If "
        "--files-to-wrap is not given, it will try to detect all "
        "executable files in this directory and create a wrapper for "
        "each of them. By default the wrapper will execute the absolute "
        "path.",
    )
    parser.add_argument(
        "--use-basename",
        ...
        "executable basename instead of the absolute path. Make sure "
        "that the wrappers behave appropriately as this depends on the "
        "PATH variable.",
    )
    parser.add_argument(
        "--use-exec",
        action="store_true",
        required=False,
        help="Prepend the command with exec, so that the wrapping process "
        "will be replaced with the target process",
    )
    parser.add_argument(
        "--inline",
        action="store_true",
        required=False,
        help="By default a generic run-in script will be created and all "
        "wrappers will delegate the actual work to it. If --inline is "
        "passed, the run-in script will not be created and the code "
        "will be inlined in each script. Useful if you want to create "
        "wrappers from many different environments on the same dest dir",
    )
    parser.add_argument(
        "-f",
        "--files-to-wrap",
        type=str,
        required=False,
        help="List of files separated by colon (:). If given only wrappers "
        "for files listed here will be created. By default the wrapper "
        "will execute the exact path that is passed to --files-to-wrap. "
        "If --bin-dir is given, it will be used as a prefix to "
        "--files-to-wrap",
    )
    parser.add_argument(
        "-d",
    ...
    conda_group.add_argument(
        "--virtual-env-dir", type=str, help="The path to the virtualenv directory"
    )

    schroot_group = parser.add_argument_group("schroot")
    schroot_name_group = schroot_group.add_mutually_exclusive_group()
    schroot_name_group.add_argument(
        "--schroot-name", type=str, help="Name of an existing schroot"
    )
    schroot_name_group.add_argument(
        "--schroot-session", type=str, help="Name of an existing schroot session"
    )
    schroot_group.add_argument(
        "--schroot-options",
        type=str,
    ...
    custom_group.add_argument(
        ...
        type=str,
        required=False,
        help="This specifies a custom script which will be used to execute "
    ...
    files_to_wrap = get_files_to_wrap(
        args.bin_dir, args.files_to_wrap, args.use_basename
    )

    kwargs = dict(inline=args.inline, command="exec " if args.use_exec else "")
    ...
    if wrapper_type == "conda":
        ...
        create_conda_wrappers(
            files_to_wrap, args.dest_dir, args.conda_env_dir, **kwargs
        )
    elif wrapper_type == "schroot":
        if not args.schroot_name and not args.schroot_session:
            print(
                "Missing schroot argument: should pass either --schroot-name "
                "or --schroot-session"
            )
            parser.print_usage()
            exit(1)
        create_schroot_wrappers(
            files_to_wrap,
            args.dest_dir,
        ...
        if not args.virtual_env_dir:
            ...
            parser.print_usage()
            exit(1)
        create_virtualenv_wrappers(
            files_to_wrap, args.dest_dir, args.virtual_env_dir, **kwargs
        )
        ...
        if not args.custom_script:
            ...
            parser.print_usage()
            exit(1)
        create_custom_wrappers(
            files_to_wrap, args.dest_dir, args.custom_script, **kwargs
        )
    else:
        print("Invalid wrapper type: {}".format(wrapper_type))
        parser.print_usage()
...
def create_conda_wrappers(files_to_wrap, destination_dir, conda_env_dir, **kwargs):
    os.path.exists(destination_dir) or os.makedirs(destination_dir)
    conda_env_dir = os.path.abspath(conda_env_dir)

    this_dir = os.path.dirname(__file__)

    run_in_template_filename = os.path.join(
        this_dir, "templates", "conda", "run-in" + get_wrapper_extension()
    )

    def template_function(content):
        content = content.replace("__CONDA_PREFIX__", conda_env_dir)
        content = content.replace(
            "__CONDA_DEFAULT_ENV__", os.path.basename(conda_env_dir)
        )
        return content
...
def create_virtualenv_wrappers(
    files_to_wrap, destination_dir, virtual_env_dir, **kwargs
):
    os.path.exists(destination_dir) or os.makedirs(destination_dir)
    ...
    run_in_template_filename = os.path.join(
        this_dir, "templates", "virtualenv", "run-in" + get_wrapper_extension()
    )
    _create_wrappers(
        files_to_wrap,
        destination_dir,
...
def create_schroot_wrappers(
    ...
    **kwargs
    ...
    run_in_template_filename = os.path.join(
        get_templates_dir(), "schroot", "run-in" + get_wrapper_extension()
    )
    ...
    def create_content(content):
        if schroot_session:
            return content.replace(
                "__CHROOT_OPTIONS__",
                "{}-r -c {}".format(schroot_options, schroot_session),
            )
        else:
            return content.replace(
                "__CHROOT_OPTIONS__", "{}-c {}".format(schroot_options, schroot_name)
            )
...
def create_custom_wrappers(files_to_wrap, destination_dir, custom_script, **kwargs):
    ...
    _create_wrappers(
        files_to_wrap, destination_dir, custom_script, lambda content: content, **kwargs
    )
...
def _create_wrappers(
    ...
    if not inline:
        run_in_filename = os.path.join(
            destination_dir, "run-in" + get_wrapper_extension()
        )
    ...
    for filename in files_to_wrap:
        ...
        if inline:
            content = run_in_template.replace("__COMMAND__", command + filename + " ")
        else:
            content = get_wrapper_template().format(
                command=command, run_in_file=run_in_filename, wrapped_file=filename
            )
...
def get_wrapper_template():
    if sys.platform == "win32":
        return """@echo off
{command}"{run_in_file}" "{wrapped_file}" %*"""
    else:
        return """#!/bin/sh
{command}"{run_in_file}" "{wrapped_file}" "$@"
"""
...
def list_executable_files(directory):
    return sorted([f for f in glob.glob(directory + "/*") if is_executable(f)])
